...
def Unet(dropout_val=0.05, batch_norm=True):
    ...
    up6 = merge([conv5, UpSampling2D(size=(2, 2))(conv6)], mode='concat', concat_axis=3)
    conv7 = double_conv_layer(up6, 512, dropout_val, batch_norm)

    up7 = merge([conv4, UpSampling2D(size=(2, 2))(conv7)], mode='concat', concat_axis=3)
    conv8 = double_conv_layer(up7, 256, dropout_val, batch_norm)

    up8 = merge([conv3, UpSampling2D(size=(2, 2))(conv8)], mode='concat', concat_axis=3)
    conv9 = double_conv_layer(up8, 128, dropout_val, batch_norm)

    up9 = merge([conv2, UpSampling2D(size=(2, 2))(conv9)], mode='concat', concat_axis=3)
    conv10 = double_conv_layer(up9, 64, dropout_val, batch_norm)

    up10 = merge([conv1, UpSampling2D(size=(2, 2))(conv10)], mode='concat', concat_axis=3)
    conv11 = double_conv_layer(up10, 32, 0, batch_norm)

    conv12 = Convolution2D(1, 1, OUTPUT_MASK_CHANNELS)(conv11)
